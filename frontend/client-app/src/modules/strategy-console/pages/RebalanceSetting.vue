        ...
        <div class="strategy-info-cards">
          <div class="info-card">
            ...
            <div class="content">
              ...
              <div class="value">{{ strategyType }}</div>
            </div>
          </div>
...
const runBacktest = async () => {
  ...
  try {
    ...
    if (!backtestDateRange.value || backtestDateRange.value.length !== 2) {
      ElMessage.error('请选择有效的回测时间范围');
      return;
    }
    
    // 处理日期格式（支持字符串和Date对象）
    let startDateStr, endDateStr;
    if (typeof backtestDateRange.value[0] === 'string') {
      startDateStr = backtestDateRange.value[0];
      endDateStr = backtestDateRange.value[1];
    } else {
      startDateStr = backtestDateRange.value[0].toISOString().split('T')[0];
      endDateStr = backtestDateRange.value[1].toISOString().split('T')[0];
    }
    ...
    console.log('回测请求参数:', requestData); // 调试日志
    
    // 发送POST请求到后端回测接口，添加时间戳避免缓存
    const response = await http.post('/strategy-rebalance/HuiCe', requestData, {
      headers: { 
    ...
    const errorMessage = error.response?.data?.message || error.message || '回测失败，请重试';
    ElMessage({
...
const handleStrategyChange = async (strategyId) => {
  ...
  try {
    ...
    const [strategyResponse, rebalanceResponse] = await Promise.all([
      http.post('/strategy-monitoring/Metrics', {
        id: strategyId,
        name: 'fu4geliu'
      }, {
        headers: { 'Content-Type': 'application/json' }
      }),
      http.post('/strategy-rebalance/Detail', {
        id: strategyId,
        name: 'fu4geliu'
      }, {
        headers: { 'Content-Type': 'application/json' }
    ...
    // 获取策略类型 - 从策略列表中找到对应的策略类型
    const currentStrategy = strategies.value.find(s => s.id === strategyId)
    strategyType.value = currentStrategy?.type || '未知'
    runtime.value = (strategyData.uptimeDays || 0) + '天'
    // 直接使用后端返回的风险等级，不做转换
    ...
    rebalance.value = {
      activeRebalancing: rebalanceData.isActiveRebalance !== undefined ? rebalanceData.isActiveRebalance : (rebalanceData.activeRebalancing !== undefined ? rebalanceData.activeRebalancing : true),
      frequency: rebalanceData.rebalancePeriod || rebalanceData.frequency || 'monthly',
      maxAdjustmentRate: rebalanceData.maxRebalanceRatio || rebalanceData.maxAdjustmentRate || 100
    }
...
const saveConfig = async () => {
  try {
    ...
    const requestData = {
      strategyId: strategyId1.value,
      rebalancePeriod: rebalance.value.frequency,
      maxRebalanceRatio: rebalance.value.maxAdjustmentRate,
      isActiveRebalance: rebalance.value.activeRebalancing
    };
    
    // 发送POST请求到后端
    const response = await http.post('/strategy-rebalance/Update', requestData, {
      headers: { 'Content-Type': 'application/json' }
    });
    ...
    ElMessage({
      message: '保存配置失败: ' + (error.response?.data?.message || error.message || '请重试'),
      type: 'error'
    });
  }
};
