    ...
    <el-container class="container">
      ...
      <el-aside width="250px" class="sidebar">
        ...
        <div class="nav-container">
          ...
          <div 
            class="nav-item"
            :class="{ active: activePath === '/strategy/rebalance' }"
            @click="navigateTo('/strategy/rebalance')"
          >
...
const getUsernameQuery = () => {
  const queryUsername = router.currentRoute.value.query.username
  if (queryUsername) return { username: queryUsername }
  if (username.value) return { username: username.value }
  return { username: '~' }
}
...
onMounted(async () => {
  try {
    ...
    if (resp.data && resp.data.success) {
      username.value = resp.data.username || '游客'
    } else {
      username.value = '游客'
    }
  } catch (e) {
